...
def calculate_fast_ordinal_power(df, woe_column, target_column, plot=False, figsize=(10, 6)):
    """
    Calculate Fast Ordinal Power (FOP) to assess the performance of WoE binning.
    
    FOP is a metric that measures how well the ordering of WoE values aligns with the true probabilities.
    It's based on the Spearman rank correlation between WoE values and the target variable.
    
    Parameters:
    -----------
    df : pandas.DataFrame
        DataFrame containing the WoE values and target variable
    woe_column : str
        Name of the column containing WoE values
    target_column : str
        Name of the column containing target variable (0/1)
    plot : bool
    ...
    --------
    fop_score : float
        The Fast Ordinal Power score (-1 to 1, higher is better)
    auc_score : float
        The ROC AUC score for comparison
    """
    if woe_column not in df.columns or target_column not in df.columns:
        raise ValueError(f"Columns {woe_column} and/or {target_column} not found in DataFrame")
    
    # Calculate Fast Ordinal Power (Spearman rank correlation)
    fop_score, _ = spearmanr(df[woe_column], df[target_column])
    
    # Calculate AUC for comparison
    auc_score = roc_auc_score(df[target_column], -df[woe_column])  # Negative WoE for proper AUC direction
    
    if plot:
        # Create binned WoE values for visualization
        bins = 10
        df_sorted = df.sort_values(by=woe_column)
        df_sorted['woe_bin'] = pd.qcut(df_sorted[woe_column], bins, labels=False, duplicates='drop')
        
        # Calculate actual target rate for each bin
        bin_stats = df_sorted.groupby('woe_bin').agg({
            woe_column: 'mean',
            target_column: 'mean'
        }).reset_index()
        ...
        # Plot WoE vs actual target rate
        ax.scatter(bin_stats[woe_column], bin_stats[target_column], s=100, alpha=0.7)
        
        # Add a trendline
        z = np.polyfit(bin_stats[woe_column], bin_stats[target_column], 1)
        p = np.poly1d(z)
        ax.plot(bin_stats[woe_column], p(bin_stats[woe_column]), "r--", alpha=0.8)
        
        for i, row in bin_stats.iterrows():
            ax.annotate(
                f"Bin {i}",
                (row[woe_column], row[target_column]),
                textcoords="offset points",
                xytext=(0, 10),
                ha='center'
            )
        ...
        ax.set_title(f"Fast Ordinal Power Assessment\nFOP: {fop_score:.4f}, AUC: {auc_score:.4f}")
        
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.show()
    
    return fop_score, auc_score